...
<fieldset id="sch_res_detail">
    ...
    <span class="sch_wr mt-2 md:mt-0">
        <input type="text" name="stx" value="<?php echo $text_stx ?>" id="stx" required class="frm_input" size="40">
        <button type="submit" class="leading-9 h-9 px-3 text-center font-normal border-0 text-base transition transition-colors duration-300 ease-out text-white bg-gray-500 hover:bg-gray-600 rounded ml-0 md:ml-2 mt-3 md:mt-0 align-middle"><i class="fa fa-search" aria-hidden="true"></i> 검색</button>
<!--         btn_submit ml-0 md:ml-2 mt-3 md:mt-0 -->
    </span>
	...
	<div class="switch_field">
    	...
    	<label for="sop_and" class="">AND</label>
...
<div id="sch_result">
    ...
    <section id="sch_res_ov">
        <h2 class="float-left inline-block text-base leading-10 font-normal mt-2"><span class="text-red-400"><?php echo $stx ?></span> 전체검색 결과</h2>
        <ul>
    ...
    for ($idx=$table_index, $k=0; $idx<count($search_table) && $k<$rows; $idx++) {
     ?>
		<div class="search_board_result mt-4">
        <h2><a href="<?php echo get_pretty_url($search_table[$idx], '', $search_query); ?>" class="text-base text-indigo-500 font-normal"><?php echo $bo_subject[$idx] ?> 게시판 내 결과</a></h2>
		<a href="<?php echo get_pretty_url($search_table[$idx], '', $search_query); ?>" class="sch_more flex">더보기</a>
        <ul>
        <?php
        for ($i=0; $i<count($list[$idx]) && $k<$rows; $i++, $k++) {
         ...
         ?>
            ...
            <li>
                <div class="sch_tit text-base">
                    <a href="<?php echo $list[$idx][$i]['href'] ?><?php echo $comment_href ?>" class="sch_res_titleinline-block my-2 text-base text-black font-semibold"><?php echo $comment_def ?><?php echo $list[$idx][$i]['subject'] ?></a>
